...
class CallingCachedMethodsTest extends Test
{
    public function test_items_cached_successfully(): void
    {
        ...
        $cachedReturn = app(TestingClass::class)->cacheCall('getSubscriber', ['id' => 1]);
        $this->assertEquals($expectedReturn, $cachedReturn);
        $this->assertEquals($expectedReturn, cache()->get('getSubscriber'));
        $this->assertNull(cache()->tags('getSubscriber')->get('getSubscriber'));
    }
    ...
    public function test_user_can_use_taggable_cache(): void
    {
        ...
        $cachedReturn = app(TestingClass::class)->cacheCall('getSubscriber', ['id' => 1]);
        $this->assertEquals($expectedReturn, $cachedReturn);
        $this->assertEquals($expectedReturn, cache()->tags('getSubscriber')->get('getSubscriber'));
        $this->assertNull(cache()->get('getSubscriber'));
    }
